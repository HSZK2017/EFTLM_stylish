...
@OnlyIn(Dist.CLIENT)
public class SDTMiniEffek {
   public static final ResourceLocation SDTMINI_EFFEK = new ResourceLocation("invincible_dmc", "sdt_small");

   public static void playSDT(SDTMiniEffek.Type type, Level level, double x, double y, double z, float radius) {
      if (EffekConfig.isEnabled("sdt_mini")) {
   ...
   public static Particle createParticleWrapper(
      SDTMiniEffek.Type type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, float radius
   ) {
   ...
   public static record Type(ResourceLocation effekId, float intrinsicRadius) {
      public static final SDTMiniEffek.Type LEVEL1 = new SDTMiniEffek.Type(SDTMiniEffek.SDTMINI_EFFEK, 1.0F);
   }
}
