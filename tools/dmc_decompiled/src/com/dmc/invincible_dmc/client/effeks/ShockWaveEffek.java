...
@OnlyIn(Dist.CLIENT)
public class ShockWaveEffek {
   public static final ResourceLocation SHOCKWAVE_EFFEK = new ResourceLocation("invincible_dmc", "yamato_shockwave");

   public static void playShockWave(ShockWaveEffek.Type type, Level level, double x, double y, double z, float radius) {
      if (EffekConfig.isEnabled("shock_wave")) {
   ...
   public static Particle createParticleWrapper(
      ShockWaveEffek.Type type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, float radius
   ) {
   ...
   public static record Type(ResourceLocation effekId, float intrinsicRadius) {
      public static final ShockWaveEffek.Type LEVEL1 = new ShockWaveEffek.Type(ShockWaveEffek.SHOCKWAVE_EFFEK, 1.0F);
   }
}
