...
@OnlyIn(Dist.CLIENT)
public class PowerFloorEffek {
   public static final ResourceLocation POWERFLOOR_EFFEK = new ResourceLocation("invincible_dmc", "yamato_powerfloor");

   public static void playPowerFloor(PowerFloorEffek.Type type, Level level, double x, double y, double z, float radius) {
      if (EffekConfig.isEnabled("power_floor")) {
   ...
   public static Particle createParticleWrapper(
      PowerFloorEffek.Type type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, float radius
   ) {
      if (!EffekConfig.isEnabled("power_floor")) {
         return null;
      } else {
   ...
   public static record Type(ResourceLocation effekId, float intrinsicRadius) {
      public static final PowerFloorEffek.Type LEVEL1 = new PowerFloorEffek.Type(PowerFloorEffek.POWERFLOOR_EFFEK, 1.0F);
   }
}
