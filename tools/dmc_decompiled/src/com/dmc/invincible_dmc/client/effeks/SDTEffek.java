...
@OnlyIn(Dist.CLIENT)
public class SDTEffek {
   public static final ResourceLocation SDT_EFFEK = new ResourceLocation("invincible_dmc", "sdt");

   public static void playSDT(SDTEffek.Type type, Level level, double x, double y, double z, float radius) {
      if (EffekConfig.isEnabled("sdt")) {
   ...
   public static Particle createParticleWrapper(
      SDTEffek.Type type, ClientLevel level, double x, double y, double z, double dx, double dy, double dz, float radius
   ) {
   ...
   public static record Type(ResourceLocation effekId, float intrinsicRadius) {
      public static final SDTEffek.Type LEVEL1 = new SDTEffek.Type(SDTEffek.SDT_EFFEK, 1.0F);
   }
}
