...
@OnlyIn(Dist.CLIENT)
public class JudgementCutSequenceDMC4Particle extends TextureSheetParticle implements JudgementCutDMC4ParticleLayer.LateRenderable {
   private static final float SOURCE_HEIGHT = 636.0F;
   private static final float WIDTH_SCALE = 1.0440252F;
   private static final float HEIGHT_SCALE = 0.9056604F;
   private final SpriteSet sprites;
   private final Quaternionf renderRotation = new Quaternionf();

   protected JudgementCutSequenceDMC4Particle(ClientLevel level, double x, double y, double z, SpriteSet sprites) {
      super(level, x, y, z);
   ...
   private void renderQuad(VertexConsumer buffer, Vec3 cameraPosition, Quaternionf cameraRotation, float partialTick) {
      ...
      float halfWidth = size * 1.0440252F;
      float halfHeight = size * 0.9056604F;
   ...
   public static class Provider implements ParticleProvider<SimpleParticleType> {
      ...
      public Particle createParticle(
         ...
         return new JudgementCutSequenceDMC4Particle(level, x, y, z, this.sprites);
      }
   }
}
