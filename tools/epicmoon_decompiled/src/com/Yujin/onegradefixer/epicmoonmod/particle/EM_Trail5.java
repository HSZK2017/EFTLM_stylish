...
public class EM_Trail5 extends AnimationTrailParticle {
   public static final ResourceLocation a = new ResourceLocation("epicmoonmod", "g");

   protected EM_Trail5(ClientLevel level, LivingEntityPatch<?> owner, Joint joint, AssetAccessor<? extends StaticAnimation> animation, TrailInfo trailInfo) {
      ...
      if (this.f_107208_.f_46443_ && !isTrailInvisible) {
         AAALevel.addParticle(
            ...
            info.position(prevStartPos)
               .position(prevEndPos)
               .rotation(-360.0F, -((float)Math.toRadians((double)((LivingEntity)((LivingEntityPatch)this.owner).getOriginal()).m_146908_())), 0.0F)
               .clone()
         );
      }
   }
            ...
            if (entitypatch == null) {
               ...
               if (animation == null) {
                  ...
                  if (trailInfo.isEmpty()) {
                     ...
                     return result.playable() ? new EM_Trail5(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, result) : null;
                  }
               }
            }
         }
