...
public class EM_Trail2 extends AnimationTrailParticle {
   public static final ResourceLocation a = new ResourceLocation("epicmoonmod", "d");

   protected EM_Trail2(ClientLevel level, LivingEntityPatch<?> owner, Joint joint, AssetAccessor<? extends StaticAnimation> animation, TrailInfo trailInfo) {
   ...
   protected void createNextCurve() {
      ...
      boolean isTrailInvisible = ((DynamicAnimation)animPlayer.getAnimation().get()).isLinkAnimation()
      ...
      ParticleEmitterInfo info = ParticleEmitterInfo.create(this.f_107208_, a);
      if (this.f_107208_.f_46443_ && !isTrailInvisible) {
         AAALevel.addParticle(
            this.f_107208_,
            true,
            info.position(prevStartPos)
               .position(prevEndPos)
               .rotation(
                  (float)Math.toRadians((double)((LivingEntity)((LivingEntityPatch)this.owner).getOriginal()).m_146909_()),
                  -((float)Math.toRadians((double)((LivingEntity)((LivingEntityPatch)this.owner).getOriginal()).m_146908_())),
                  0.0F
               )
               .clone()
         );
            ...
            if (entitypatch == null) {
               ...
               if (animation == null) {
                  ...
                  if (trailInfo.isEmpty()) {
                     ...
                     return result.playable() ? new EM_Trail2(level, entitypatch, entitypatch.getArmature().searchJointById(jointId), animation, result) : null;
                  }
               }
            }
         }
