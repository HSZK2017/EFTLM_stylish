...
public class WoopieTheSwordSkill extends WeaponInnateSkill {
   ...
   public void executeOnServer(SkillContainer skillContainer, FriendlyByteBuf friendlyByteBuf) {
      ...
      final Player player = (Player)skillContainer.getExecutor().getOriginal();
      ItemStack itemStack = player.m_21205_();
      if (itemStack.m_41720_() instanceof WoopieTheSwordItem) {
         new DelayedTask(6) {
            public void run() {
               Vec3 windPos = EpicfightUtil.getJointWithTranslation(
                  player, new Vec3f(0.0F, 0.0F, 0.0F), ((HumanoidArmature)Armatures.BIPED.get()).toolR, 5.3F, 0.5
               );
               if (windPos != null) {
                  BlockPos mutePos = BlockPos.m_274446_(windPos);
                  AnnoyingVillagers.PACKET_HANDLER
                     .send(PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> player), new ClientboundMuteExplosionAtPos(mutePos, 4));
                  player.m_9236_().m_255391_(player, windPos.f_82479_, windPos.f_82480_, windPos.f_82481_, 2.0F, false, ExplosionInteraction.NONE);
                  AnnoyingVillagers.PACKET_HANDLER
                     .send(PacketDistributor.TRACKING_ENTITY_AND_SELF.with(() -> player), new ClientboundWoopieSwordWindFx(windPos));
               }
            }
         };
      }
