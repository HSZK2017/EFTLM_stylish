...
public class DiamondAttractorParticleEmitterInfo extends ParticleEmitterInfo {
   ...
   public DiamondAttractorParticleEmitterInfo followSword(Entity entity, int durationTicks, Vec3f swordLocalOffset) {
      ...
      this.useEyePosition = false;
      this.swordLocalOffset = swordLocalOffset == null ? new Vec3f(0.0F, 0.0F, 0.0F) : swordLocalOffset;
      return this;
   }
   ...
   private Vec3 getSwordPosition(Entity entity, float partialTick) {
      try {
         return EpicfightUtil.getJointWithTranslation(entity, this.swordLocalOffset, ((HumanoidArmature)Armatures.BIPED.get()).toolR, partialTick, 0.0);
   ...
   private Vec3 getCurrentFollowPosition(float partialTick) {
      ...
      if (entity != null) {
         if (entity.m_6084_() && !entity.m_213877_()) {
            Vec3 pos = this.getSwordPosition(entity, partialTick);
            if (pos == null) {
               pos = getInterpolatedEntityPosition(entity, partialTick);
            }

            return pos.m_82549_(this.offset);
         } else {
            return null;
         }
