...
public class HeavyAttackAnimation extends AttackAnimation {
   void init() {
      ...
      if (FMLEnvironment.dist == Dist.CLIENT) {
         ...
         JointMaskEntry jointMaskEntry = JointMaskEntry.builder().defaultMask(JointMaskSet.of(jointMaskSet)).create();
         this.addProperty(ClientAnimationProperties.JOINT_MASK, jointMaskEntry);
      }
   }

   public HeavyAttackAnimation(
      float convertTime,
      float antic,
      float preDelay,
      float contact,
      ...
      Joint colliderJoint,
      AnimationAccessor<? extends HeavyAttackAnimation> accessor,
   ...
   public HeavyAttackAnimation(
      float convertTime, AnimationAccessor<? extends HeavyAttackAnimation> accessor, AssetAccessor<? extends Armature> armature, Phase... phases
   ) {
   ...
   protected void bindPhaseState(Phase phase) {
      ...
      this.stateSpectrumBlueprint
         ...
         .newTimePair(f, phase.contact + 0.01F)
   ...
   public void end(LivingEntityPatch<?> livingentitypatch, AssetAccessor<? extends DynamicAnimation> nextAnimation, boolean flag) {
      ...
      boolean flag1 = ((BooleanValue)((LivingEntity)livingentitypatch.getOriginal())
            .m_9236_()
            .m_46469_()
            .m_46170_(EpicFightGameRules.STIFF_COMBO_ATTACKS.getRuleKey()))
         .m_46223_();
      if (!flag && !((DynamicAnimation)nextAnimation.get()).isMainFrameAnimation() && livingentitypatch.isLogicalClient() && !flag1) {
         float f = 0.05F * this.getPlaySpeed(livingentitypatch, (DynamicAnimation)nextAnimation.get());
   ...
   public TypeFlexibleHashMap<StateFactor<?>> getStatesMap(LivingEntityPatch<?> livingentitypatch, float f) {
      ...
      if (!((BooleanValue)((LivingEntity)livingentitypatch.getOriginal()).m_9236_().m_46469_().m_46170_(EpicFightGameRules.STIFF_COMBO_ATTACKS.getRuleKey()))
         .m_46223_()) {
         typeflexiblehashmap.put(EntityState.MOVEMENT_LOCKED, Boolean.FALSE);
         typeflexiblehashmap.put(EntityState.UPDATE_LIVING_MOTION, Boolean.TRUE);
      }
   ...
   public boolean shouldPlayerMove(LocalPlayerPatch localplayerpatch) {
      return !localplayerpatch.isLogicalClient()
         || ((BooleanValue)((LocalPlayer)localplayerpatch.getOriginal()).m_9236_().m_46469_().m_46170_(EpicFightGameRules.STIFF_COMBO_ATTACKS.getRuleKey()))
            .m_46223_()
         || ((LocalPlayer)localplayerpatch.getOriginal()).f_108618_.f_108567_ == 0.0F
            && ((LocalPlayer)localplayerpatch.getOriginal()).f_108618_.f_108566_ == 0.0F;
   }
}
