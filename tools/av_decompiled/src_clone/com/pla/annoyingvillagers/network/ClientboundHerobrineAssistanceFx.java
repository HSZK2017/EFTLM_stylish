...
public record ClientboundHerobrineAssistanceFx(Vec3 from) {
   ...
   public static void handle(ClientboundHerobrineAssistanceFx msg, Supplier<Context> ctx) {
      Context c = ctx.get();
      c.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> ClientPacketHandlers.handleHerobrineAssistanceFx(msg)));
